...
def display_recommendations(recs, name):
    print( "\n AI-Analyzed Movie Recommendations :")
    for idx, (title, polarity) in enumerate(recs, 1):
        sentiment = "Positive" if polarity > 0 else "Negative" if polarity < 0 else "Neutral"
        print(f"{Fore.GREEN}{idx}. {title} (Polarity: {polarity:.2f}, {sentiment})")
...
def handle_ai(name):
    ...
    while True:
        rating_input = input( "Enter minimum IMDB rating (7.6-9.3) or 'skip': ").strip()
    ...
    recs = recommend_movies(genre=genre, mood=mood, rating=rating, top_n=2)
